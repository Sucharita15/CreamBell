CREATE PROCEDURE [dbo].[SP_SALEREGISTER]
...
BEGIN INSERT INTO #tmpBuCode SELECT BU_CODE FROM AX.ACXSITEBUMAPPING WHERE SITEID = @SiteId
...
PRODUCT_SUBCATEGORY,PRODUCT_CODE,PRODUCT_NAME,sum(LTR)as LTR,SUM(AMOUNT)AS AMOUNT
 ...
 FROM vw_SaleRegister  WHERE [DIST. CODE]=@SiteId AND [INVOICE DATE]>=@StartDate AND [INVOICE DATE]<=@EndDate 
