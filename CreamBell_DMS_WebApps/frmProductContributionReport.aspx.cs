...
{
    public partial class frmProductContributionReport : System.Web.UI.Page
    {
        ...
        private void ShowReportByFilter()
        {
            ...
            try
            {
                ...
                query = "dbo.ACX_USP_PRODUCTCONTRIBUTIONREPORT";
                String usertype = Convert.ToString(Session["LOGINTYPE"]);
                ...
                //string SiteId = ddlSiteId.SelectedItem.Value.ToString();
                //string Category = drpCategory.SelectedItem.Value.ToString();
                //string SubCategory = drpSubCategory.SelectedItem.Value.ToString();
                //string Product = drpProduct.SelectedItem.Value.ToString();
        ...
        private   void LoadDataInReportViewer(DataTable dtSetData)
        {
            try
            {
                if (dtSetData.Rows.Count > 0)
                {
                    ...
                    DataSetParameter.Columns.Add("Category");
                    
                  
                    DataSetParameter.Columns.Add("SubCategory");
                    DataSetParameter.Columns.Add("Product");
                    ...
                    {
                        DataSetParameter.Rows[0]["StateCode"] = ddlState.SelectedItem.Value;
                    }
                    if (ddlSiteId.SelectedIndex != -1)
                    {
                        ...
                        {
                            DataSetParameter.Rows[0]["SITEID"] = ddlSiteId.SelectedItem.Value;
                        }
                    }
                    if (drpCategory.SelectedIndex >= 1)
                    {
                        //cmd.Parameters.AddWithValue("@Category", drpCategory.SelectedItem.Value);
                        DataSetParameter.Rows[0]["Category"] = drpCategory.SelectedItem.Value;
                    }
                    else
                    {
                        DataSetParameter.Rows[0]["Category"] = "";
                    }
                    if (drpSubCategory.SelectedIndex >= 1)
                    {
                        //cmd.Parameters.AddWithValue("@SubCategory", drpSubCategory.SelectedItem.Value);
                        DataSetParameter.Rows[0]["SubCategory"] = drpSubCategory.SelectedItem.Value;
                    }
                    else
                    {
                        DataSetParameter.Rows[0]["SubCategory"] = "";
                    }
                    if (drpProduct.SelectedIndex >= 1)
                    {
                        //cmd.Parameters.AddWithValue("@Product", drpProduct.SelectedItem.Value);
                        DataSetParameter.Rows[0]["Product"] = drpProduct.SelectedItem.Value;
                    }
                    else
                    {
                        //cmd.Parameters.AddWithValue("@Product", "");
                        DataSetParameter.Rows[0]["Product"] = "";
                    }
                    if (drpCustGroup.SelectedIndex >= 1)
                    {
                        //cmd.Parameters.AddWithValue("@CustGroup", drpCustGroup.SelectedItem.Value);
                        DataSetParameter.Rows[0]["CustGroup"] = drpCustGroup.SelectedItem.Value;
                    }
                    else
                    {
                        DataSetParameter.Rows[0]["CustGroup"] = "";
                    }
                    DataSetParameter.Rows[0]["UserType"] = Convert.ToString(Session["LOGINTYPE"]);
                    DataSetParameter.Rows[0]["UserCode"] = Convert.ToString(Session["USERID"]);
                    ReportViewer1.LocalReport.ReportPath = Server.MapPath("Reports\\ProductContributionReport.rdl");
        ...
        protected void ddlCountry_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (objcheckSitecode != null)
            {
               ...
               // baseObj.BindToDropDown(ddlSiteId, sqlstr1, "Name", "Code");
                CreamBell_DMS_WebApps.App_Code.CreamBellFramework.BindToDropDown(ddlSiteId, sqlstr1, "Name", "Code");
            }
            else
            {
                ddlSiteId.Items.Clear();
        ...
        protected void fillSiteAndState(DataTable dt)
        {
            ...
            {
                sqlstr = "Select Distinct I.StateCode Code,I.StateCode+'-'+LS.Name as Name from [ax].[INVENTSITE] I left join [ax].[LOGISTICSADDRESSSTATE] LS on LS.STATEID = I.STATECODE where I.STATECODE <>'' ";
                //ddlState.Items.Add("Select...");
