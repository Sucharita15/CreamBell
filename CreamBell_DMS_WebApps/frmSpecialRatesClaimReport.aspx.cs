...
{
    public partial class frmDiscountClaimCalculationReport : System.Web.UI.Page
    {
        ...
        {
            string StateList = "";
            foreach (ListItem item in chkListState.Items.Cast<ListItem>().Where(item => item.Selected))
            {
                //if (item.Selected)
                //{
                if (StateList == "")
        ...
        protected void FillBusinessGroup()
        {
            string Bucode = "";
            if (Convert.ToString(Session["LOGINTYPE"]) == "3") {
                Bucode = ucRoleFilters.GetCommaSepartedSiteId();
            } else {
                foreach (ListItem item in chkListSite.Items.Cast<ListItem>().Where(item => item.Selected))
                {
                    //if (item.Selected)
                    //{
                    if (Bucode == "")
                    {
                        Bucode += "'" + item.Value.ToString() + "'";
                    }
                    else
                    {
                        Bucode += ",'" + item.Value.ToString() + "'";
            ...
            //DataTable dt = new DataTable();
            string query = "select distinct bm.bu_code,bu_desc from ax.acxsitebumapping sbp join ax.ACXBUMASTER bm on bm.bu_code = sbp.BU_CODE where SITEID in (" + Bucode + ")";
            //dt = new DataTable();
            DataTable dt = baseObj.GetData(query);
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                CheckBoxList1.DataTextField = "bu_desc";
                CheckBoxList1.DataValueField = "bu_code";
        ...
        private bool ValidateInput()
        {
            ...
            string buunit = "";
            foreach (ListItem items in CheckBoxList1.Items.Cast<ListItem>().Where(item => item.Selected))
            {
                if (buunit == "")
                {
            ...
            {
                b = false;
                LblMessage.Text = "Please Provide Business Unit";
            }
        ...
        private void ShowReportByFilter()
        {
            ...
            try
            {
                ...
                //query = "[ax].[ACX_SpecialRatesClaimReport]";
                cmd.CommandText = "[ax].[ACX_SpecialRatesClaimReport]";
                ...
                foreach (ListItem item in chkListCustomerGroup.Items.Cast<ListItem>().Where(item => item.Selected))
                {
                    //if (item.Selected)
                    //{
                    if (CustGroupList == "")
                ...
                //conn = new SqlConnection(objGlobal.GetConnection());
        ...
        private void LoadDataInReportViewer(DataTable dtSetData)
        {
            try
            {
                if (dtSetData.Rows.Count > 0)
                {
                    ...
                    ReportViewer1.LocalReport.ReportPath = Server.MapPath("Reports\\SpecialRatesClaimReport.rdl");
