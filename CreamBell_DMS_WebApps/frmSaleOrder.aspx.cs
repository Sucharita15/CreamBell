...
namespace CreamBell_DMS_WebApps
{
    ...
    public partial class frmSaleOrder : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            chkCompositionSchemeNew.Enabled = false;
        ...
        public void getData() {
            ...
            SqlParameter sitecodeParam = new SqlParameter("@SiteCode", SqlDbType.NVarChar, 50);
            sitecodeParam.Value = Convert.ToString(Session["SiteCode"]);
            sqlParameters.Add(sitecodeParam);

            SqlParameter DATAAREAIDParam = new SqlParameter("@DATAAREAID", SqlDbType.NVarChar, 50);
            DATAAREAIDParam.Value = Session["DATAAREAID"].ToString();
            sqlParameters.Add(DATAAREAIDParam);
            DataSet ds = CreamBellFramework.GetDataSetFromStoredProcedure("USP_GetSaleOrderData", sqlParameters.ToArray());
            Session["businessunit"] = ds.Tables[0];
            Session["customergroup"] = ds.Tables[1];

        }
        public void fillBusinessUnit()
        {
            if (Session["businessunit"] == null) { getData(); }
            ddlBusinessUnitNew.DataSource = Session["businessunit"];
            ddlBusinessUnitNew.DataTextField = "bu_desc";
            ddlBusinessUnitNew.DataValueField = "bu_code";
            ddlBusinessUnitNew.DataBind();
            ddlBusinessUnitNew.Items.Insert(0, new ListItem("All..."));

            //if (Session["businessunit"] != null) {
            //    ddlBusinessUnit.DataSource = Session["businessunit"];
            //    ddlBusinessUnit.DataTextField = "bu_desc";
            //    ddlBusinessUnit.DataValueField = "bu_code";
            //    ddlBusinessUnit.DataBind();
            //    ddlBusinessUnit.Items.Insert(0, new ListItem("All..."));
            //} else {
            ...
            //    CreamBellFramework.BindToDropDown(ddlBusinessUnit, query, "bu_desc", "bu_code","All...");
            //}
        }
        public void BindIndentDetails(string Indent_NO, string CustomerCode)
        {
            try
            {
                ...
                strQuery = "select A.SiteId as [CUSTOMER_CODE],A.ACXPlantName as SiteId " +
                        ...
                        " where A.Indent_No='" + Indent_NO + "' and A.ACXPlantName='" + Session["SiteCode"].ToString() + "' and A.SiteId='" + CustomerCode + "'  and (A.[DATAAREAID]='" + Session["DATAAREAID"].ToString() + "'  or A.DATAAREAID='VMAS')";
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    {
                        ddlCustomerGroupNew.SelectedValue = dtHeader.Rows[0]["CUSTGROUP_CODE"].ToString();
                        ddlCustomerGroup_SelectedIndexChanged(this, null);
                        ddlCustomerNew.SelectedValue = dtHeader.Rows[0]["CUSTOMER_CODE"].ToString();
        ...
        public void BindSODetailsSD(string SONO)
        {
            ...
            strQuery = "Select A.[LINE_NO] as SNO,B.[PRODUCT_GROUP] as MaterialGroup,A.[PRODUCT_CODE] as ProductCodeName,B.Product_Name,A.[CRATES] as QtyCrates ,A.[BOXQty] as OnlyQtyBox,CAST(A.[PCSQTY] AS decimal(9,2)) as QtyPcs,CAST(A.[BOX] AS decimal(9,2)) as QtyBox,A.[LTR] as QtyLtr,A.[RATE] as Price,A.[Amount] as Value,A.[UOM],case when A.[DiscType]='0' then 'Per' when A.[DiscType]='1' then 'Val' else '2' end as DiscType,A.[Disc],isnull(A.Sec_Disc_Per,0) as Sec_Disc_Per,isnull(A.SEC_DISC_AMOUNT,0) as SEC_DISC_AMOUNT,isnull(A.TDValue,0) as TDValue,isnull(A.TD_Per,0) as TD_Per,isnull(A.PEValue,0) as PE,isnull(A.PE_Per,0) as PE_Per," +
                     ...
                     " where A.SO_NO='" + SONO + "' and A.SiteId='" + Session["SiteCode"].ToString() + "' and (A.[DATAAREAID]='" + Session["DATAAREAID"].ToString() + "'  or A.DATAAREAID='VMAS')" +
                     " Order By A.[LINE_NO] ";
        ...
        public void BindSODetails(string SONO)
        {
            ...
            strQuery = "Select SH.SiteId,SH.[CUSTOMER_CODE],SH.[PSR_CODE],SH.[PSR_BEAT],SH.[DELIVERY_DATE],CU.CUST_GROUP as CUSTGROUP_CODE " +
                       ...
                       " Where SH.SO_NO='" + SONO + "' and SH.SiteId='" + Session["SiteCode"].ToString() + "' and (SH.[DATAAREAID]='" + Session["DATAAREAID"].ToString() + "'  or SH.DATAAREAID='VMAS')";
            dt = baseObj.GetData(strQuery);
            if (dt.Rows.Count > 0)
            {
        ...
        public void FillLineItem(string Indent_NO, string CustomerCode)
        {
            try
            {
                ...
                if (ddlPSRNameNew.Visible == true)
                {
                    strPsrName = ddlPSRNameNew.SelectedItem.Value;
                    strBeatName = ddlBeatNameNew.SelectedItem.Value;
                }
                ...
                cmd.Parameters.AddWithValue("@CUSTCOMPOSITIONSCHEME", (chkCompositionSchemeNew.SelectedValue));
                cmd.Parameters.AddWithValue("@BILLTOADDRESS", txtAddress.Text);
                cmd.Parameters.AddWithValue("@SHIPTOADDRESS", ddlShipToAddressNew.SelectedValue.ToString());
                ...
                for (int i = 0; i < dtExcelData.Rows.Count; i++)
                {
                    ...
                    else if (ReturnArray != null && ReturnArray[2].ToString() != "")
                    {
                        ...
                        DataTable dt = new DataTable();
                        string abc = "EXEC USP_ACX_GetSalesLineCalcGST '" + dtExcelData.Rows[i]["ProductCode"].ToString() + "','" + ddlCustomerNew.SelectedValue.ToString() + "','" + Session["SiteCode"].ToString() + "'," + Convert.ToDecimal(dtExcelData.Rows[i]["Qty"].ToString()) + "," + Convert.ToDecimal(ReturnArray[2].ToString()) + ",'" + Session["SITELOCATION"].ToString() + "','" + ddlCustomerGroupNew.SelectedValue.ToString() + "','" + Session["DATAAREAID"].ToString() + "'";
                        ...
                        if (dt.Rows.Count > 0)
                        {
                            ...
                            cmd1.Parameters.AddWithValue("@CUSTOMER_CODE", ddlCustomerNew.SelectedItem.Value);
                            ...
                            cmd1.Parameters.AddWithValue("@CompositionScheme", (chkCompositionSchemeNew.SelectedValue));
        ...
        protected void ddlProductGroup_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                ilist.Add("@ProductGroup"); litem.Add(DDLProductGroupNew.SelectedValue.ToString());
                DataTable dtBinddropdown = baseObj.GetData_New(query, CommandType.StoredProcedure, ilist, litem);
                DDLProductSubCategoryNew.DataSource = dtBinddropdown;
                DDLProductSubCategoryNew.DataTextField = "CODE";
                DDLProductSubCategoryNew.DataValueField = "CODE_DISC";
                DDLProductSubCategoryNew.DataBind();
                DDLProductSubCategoryNew.Items.Insert(0, new ListItem("--Select--", ""));
                FillProductCode();
                DDLProductSubCategoryNew.Focus();
                txtCrate.Text = "";
                txtPcs.Text = "";
        ...
        public void PcsBillingApplicable()
        {
            ...
            DataTable dt = baseObj.GetPcsBillingApplicability(Session["SCHSTATE"].ToString(), DDLProductGroupNew.SelectedItem.Value);  //st.Substring(st.Length - 6) + System.DateTime.Now.ToString("yyMMddhhmmss");
        ...
        {
            if (Session["customergroup"] == null) { getData(); }
            ddlCustomerGroupNew.DataSource = Session["customergroup"];
            ddlCustomerGroupNew.DataTextField = "Name";
            ddlCustomerGroupNew.DataValueField = "CUSTGROUP_CODE";
            ddlCustomerGroupNew.DataBind();
            ddlCustomerGroupNew.Items.Insert(0, new ListItem("Select..."));

            //if (Session["customergroup"] != null)
            //{
            //    ddlCustomerGroup.DataSource = Session["customergroup"];
            //    ddlCustomerGroup.DataTextField = "Name";
            //    ddlCustomerGroup.DataValueField = "CUSTGROUP_CODE";
            //    ddlCustomerGroup.DataBind();
            //    ddlCustomerGroup.Items.Insert(0, new ListItem("Select..."));
            //}
            //else
            //{
            //    getData();
            //    strQuery = "Select CUSTGROUP_CODE+'-'+CUSTGROUP_NAME as Name,CUSTGROUP_CODE from ax.ACXCUSTGROUPMASTER where (DATAAREAID ='" + Session["DATAAREAID"].ToString() + "'  or DATAAREAID='VMAS') and  Blocked = 0";
            //    ddlCustomerGroup.Items.Clear();
            //    CreamBellFramework.BindToDropDown(ddlCustomerGroup, strQuery, "Name", "CUSTGROUP_CODE", "Select...");
            //}
        ...
        public void ProductGroup()
        {
            ...
            ilist.Add("@USERCODE"); litem.Add(Session["USERID"].ToString());
            if (ddlBusinessUnitNew.SelectedItem == null  ||  ddlBusinessUnitNew.SelectedItem.Text == "All...")
            {
                ilist.Add("@BU_CODE"); litem.Add("");
            }
            else
            {
                ilist.Add("@BU_CODE"); litem.Add(ddlBusinessUnitNew.SelectedItem.Value.ToString());
            }
            ...
            DDLProductGroupNew.DataSource = dtBinddropdown;
            DDLProductGroupNew.DataTextField = "CODE";
            DDLProductGroupNew.DataValueField = "CODE_DESC";
            DDLProductGroupNew.DataBind();
            DDLProductGroupNew.Items.Insert(0, new ListItem("--Select--", ""));
        }
        //public void ProductGroup()
        //{
        ...
        public void ProductSubCategory()
        {
            strQuery = @"Select distinct P.PRODUCT_SUBCATEGORY as Name,P.PRODUCT_SUBCATEGORY as Code from ax.InventTable P WHERE P.block=0 "
                        + " and P.PRODUCT_GROUP='" + DDLProductGroupNew.SelectedItem.Value + "'";
            DDLProductSubCategoryNew.Items.Clear();
            DDLProductSubCategoryNew.Items.Add("Select...");
            baseObj.BindToDropDownp(DDLProductSubCategoryNew, strQuery, "Name", "Code");
            // FillProductCode();
            DDLProductSubCategoryNew.Focus();
        }
        ...
        private DataTable AddLineItems()
        {
            ...
            try
            {
                ...
                DataRow[] dataPerDay = (from myRow in dtLineItems.AsEnumerable()
                                        where myRow.Field<string>("ProductCodeName") == DDLMaterialCodeNew.SelectedValue.ToString()
                ...
                if (dataPerDay.Count() == 0)
                {
                    ...
                    DataTable dt = new DataTable();
                    string st123 = "EXEC USP_ACX_GetSalesLineCalcGST '" + DDLMaterialCodeNew.SelectedValue.ToString() + "','" + ddlCustomerNew.SelectedValue.ToString() + "','" + Session["SiteCode"].ToString() + "'," + Convert.ToDecimal(strtxtBoxQty.Trim()) + "," + Convert.ToDecimal(txtPrice.Text.Trim()) + ",'" + Session["SITELOCATION"].ToString() + "','" + ddlCustomerGroupNew.SelectedValue.ToString() + "','" + Session["DATAAREAID"].ToString() + "'";
                    dt = baseObj.GetData("EXEC USP_ACX_GetSalesLineCalcGST '" + DDLMaterialCodeNew.SelectedValue.ToString() + "','" + ddlCustomerNew.SelectedValue.ToString() + "','" + Session["SiteCode"].ToString() + "'," + Convert.ToDecimal(strtxtBoxQty.Trim()) + "," + Convert.ToDecimal(txtPrice.Text.Trim()) + ",'" + Session["SITELOCATION"].ToString() + "','" + ddlCustomerGroupNew.SelectedValue.ToString() + "','" + Session["DATAAREAID"].ToString() + "'");
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        ...
                        row["SNO"] = count;
                        row["MaterialGroup"] = DDLProductGroupNew.SelectedItem.Text.ToString();
                        row["ProductCodeName"] = DDLMaterialCodeNew.SelectedValue.ToString();
                        row["Product_Name"] = DDLMaterialCodeNew.SelectedItem.Text.ToString();
                        ...
                        row["QtyBox"] = decimal.Parse(strtxtBoxQty.Trim().ToString());
                        ...
                        if (Session["LineItem"] == null)
                        {
                            ...
                            cmd.Parameters.Clear();
                            cmd.Parameters.AddWithValue("@statusProcedure", "Insert");
                            cmd.Parameters.AddWithValue("@CUSTOMER_CODE", ddlCustomerNew.SelectedItem.Value);
                            cmd.Parameters.AddWithValue("@RECID", "");
                            cmd.Parameters.AddWithValue("@SO_NO", PRESO_NO);
                            cmd.Parameters.AddWithValue("@PSR_CODE", strPsrName);
                            cmd.Parameters.AddWithValue("@DELIVERY_DATE", Convert.ToDateTime(txtDeliveryDate.Text).ToString("dd-MMM-yyyy"));
                            ...
                            cmd.Parameters.AddWithValue("@BILLTOADDRESS", txtAddress.Text);
                            cmd.Parameters.AddWithValue("@SHIPTOADDRESS", ddlShipToAddressNew.SelectedItem.ToString());
                        ...
                        cmd1.Parameters.AddWithValue("@CUSTOMER_CODE", ddlCustomerNew.SelectedItem.Value);
                        cmd1.Parameters.AddWithValue("@SITEID", Session["SiteCode"].ToString());
                        cmd1.Parameters.AddWithValue("@PRODUCT_CODE", DDLMaterialCodeNew.SelectedValue.ToString());
                        cmd1.Parameters.AddWithValue("@BOX", decimal.Parse(strtxtBoxQty.Trim().ToString()));
        ...
        private bool ValidateLineItemAdd()
        {
            ...
            if (DDLProductGroupNew.Text == "Select..." || DDLProductGroupNew.Text == "")
            {
                //this.Page.ClientScript.RegisterStartupScript(GetType(), "Alert", " alert('Select Material Group !');", true);
                ...
                DDLProductGroupNew.Focus();
                b = false;
                return b;
            }
            if (ddlPSRNameNew.Visible == true)
            {
                if (ddlPSRNameNew.SelectedItem.Value == "Select..." || ddlPSRNameNew.Text == string.Empty)
                {
                ...
                if (ddlBeatNameNew.SelectedItem.Value == "Select..." || ddlBeatNameNew.Text == string.Empty)
                {
                    ...
                    ddlBeatNameNew.Focus();
                    b = false;
                    return b;
                }
            }
            if (DDLMaterialCodeNew.Text == string.Empty || DDLMaterialCodeNew.Text == "Select...")
            {
            ...
            // || txtEnterQty.Text == "0"
            if (strtxtBoxQty == string.Empty || strtxtBoxQty=="0")
            {
                b = false;
            ...
            if (txtCrate.Text == "0" && strtxtBoxQty == "0" && txtPcs.Text == "0")
            {
                b = false;
                //this.Page.ClientScript.RegisterStartupScript(GetType(), "Alert", " alert('ltr cannot be left blank !');", true);
        ...
        protected void lnkbtnDel_Click(object sender, EventArgs e)
        {
            ...
            if (Session["SONO"] != null)   // for 1st time when we save the record.
            {
                ...
                {
                    string query = "Delete From [ax].[ACXSALESLINEPRE] where [SO_NO]='" + Session["SONO"] + "' and ([DATAAREAID]='" + Session["DATAAREAID"].ToString() + "'  or DATAAREAID='VMAS') and [SITEID]='" + Session["SiteCode"].ToString() + "' and  [LINE_NO]=" + hiddenField.Value + "";
                    ...
                    strQuery = "Select A.[LINE_NO] AS SNO,B.[PRODUCT_GROUP] as MaterialGroup,A.[PRODUCT_CODE] as ProductCodeName,A.[PRODUCT_CODE] +'-'+B.Product_Name as Product_Name,A.[CRATES] as QtyCrates,CAST(A.[BOX] AS decimal(9,2)) as QtyBox,FLOOR(A.[BOX]) as OnlyQtyBox,CAST((CAST(A.[BOX] AS decimal(9,2))-CAST(FLOOR(A.[BOX]) AS DECIMAL(9,0)))*B.Product_PackSize AS decimal(9,0)) as QtyPcs,A.[LTR] as QtyLtr,A.[RATE] as Price,A.[AMOUNT] as Value,A.[UOM],case when A.[DiscType]='0' then 'Per' when A.[DiscType]='1' then 'Val' else '2' end as DiscType,A.[Disc]" +
                        ...
                        " FROM [ax].[ACXSALESLINEPRE] A " +
                        " INNER JOIN ax.InventTable B on A.[PRODUCT_CODE]=B.ITEMID" +
                        " WHERE A.SO_NO='" + Session["SONO"] + "' and A.SiteId='" + Session["SiteCode"].ToString() + "' and (A.[DATAAREAID]='" + Session["DATAAREAID"].ToString() + "'  or A.DATAAREAID='VMAS')" +
                        " ORDER BY A.[LINE_NO] ";
                    Session["LineItem"] = baseObj.GetData(strQuery);
                }
                else
            ...
            else if (Session["PreSoNO"] != null) // when we came from pending SO list.
            {
                if (gvDetails.Rows.Count > 0)
                {
                    string query = "Delete from [ax].[ACXSALESLINEPRE] where [SO_NO]='" + Session["PreSoNO"] + "' and ([DATAAREAID]='" + Session["DATAAREAID"].ToString() + "'  or DATAAREAID='VMAS') and [SITEID]='" + Session["SiteCode"].ToString() + "' and  [LINE_NO]=" + hiddenField.Value + "";
                    baseObj.ExecuteCommand(query);
                    //=================Fill Datatable After delete from Pretable========
                    //AddColumnInDataTable();
                    Session["LineItem"] = null;
                    strQuery = "SELECT A.[LINE_NO] as SNO,B.[PRODUCT_GROUP] as MaterialGroup,A.[PRODUCT_CODE] as ProductCodeName,A.[PRODUCT_CODE] +'-'+B.Product_Name as Product_Name,A.[CRATES] as QtyCrates,CAST(A.[BOX] AS decimal(9,2)) as QtyBox ,FLOOR(A.[BOX]) as OnlyQtyBox,CAST((CAST(A.[BOX] AS decimal(9,2))-CAST(FLOOR(A.[BOX]) AS DECIMAL(9,0)))*B.Product_PackSize AS decimal(9,0)) as QtyPcs,A.[LTR] as QtyLtr,A.[RATE] as Price,A.[AMOUNT] as Value,A.[UOM],case when A.[DiscType]='0' then 'Per' when A.[DiscType]='1' then 'Val' else '2' end as DiscType,A.[Disc]" +
                               ...
                               " FROM [ax].[ACXSALESLINEPRE] A " +
                               " INNER JOIN ax.InventTable B on A.[PRODUCT_CODE]=B.ITEMID" +
                               " WHERE A.SO_NO='" + Session["PreSoNO"] + "' and A.SiteId='" + Session["SiteCode"].ToString() + "' and (A.[DATAAREAID]='" + Session["DATAAREAID"].ToString() + "'  or A.DATAAREAID='VMAS')" +
                               " ORDER BY A.[LINE_NO] ";
                    Session["LineItem"] = baseObj.GetData(strQuery);
                }
        ...
        protected void BtnAddItem_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                Boolean ResetMsg;
                ResetMsg = false;
                ProcessId = DateTime.Now.ToString("yyMMddHHmmssffff");
                baseObj.DbEventWrite(ProcessId, Convert.ToString(Session["SiteCode"]), 1, "Start", 0);
                ReCalculate1();
                baseObj.DbEventWrite(ProcessId, Convert.ToString(Session["SiteCode"]), 2, "Recalculate", 0);
                ...
                if (dtDublicate != null)
                {
                    ...
                    {
                        if (dtDublicate.Select("ProductCodeName='" + DDLMaterialCodeNew.SelectedValue.ToString() + "'").Length > 0)
                        {
                            string message = "alert('" + DDLMaterialCodeNew.SelectedItem.Text + " is already exists in the list .Please Select Another Product !!');";
                            txtBoxPcs.Text = "";
                            txtViewTotalBox.Text = "";
                            txtViewTotalPcs.Text = "";
                            txtPcs.Text = "";
                ...
                baseObj.DbEventWrite(ProcessId, Convert.ToString(Session["SiteCode"]), 3, "Validation Check", 0);
                if (ddlPSRNameNew.Visible == true)
                {
                    string stringquery = "select ORDERALLOWED from ax.ACXPSRSITELinkingMaster where Site_code ='" + Session["SiteCode"].ToString() + "' and psrcode = '" + ddlPSRNameNew.SelectedItem.Value + "'";
                ...
                baseObj.DbEventWrite(ProcessId, Convert.ToString(Session["SiteCode"]), 4, "Order Allowed", 0);
                //============================
                bool valid = true;
                ...
                baseObj.DbEventWrite(ProcessId, Convert.ToString(Session["SiteCode"]), 5, "ValidateLine" + valid, 0);
                if (valid == true)
                {
                    DataTable dt = new DataTable();
                    ...
                    baseObj.DbEventWrite(ProcessId, Convert.ToString(Session["SiteCode"]), 6, "Session Get", 0);
                    dt = AddLineItems();
                    baseObj.DbEventWrite(ProcessId, Convert.ToString(Session["SiteCode"]), 7, "LineAdd", dt.Rows.Count);
                    dat1 = dt.Copy();

                    Session["LineItem"] = dt;
                    baseObj.DbEventWrite(ProcessId, Convert.ToString(Session["SiteCode"]), 8, "Start Binding", 0);
                    BindingGird();
                    baseObj.DbEventWrite(ProcessId, Convert.ToString(Session["SiteCode"]), 9, "End Binding", 0);
                    if (ResetMsg)
                    {
                ...
                baseObj.DbEventWrite(ProcessId, Convert.ToString(Session["SiteCode"]), 10, "Clear Text", 0);
                txtEnterQty.Text = string.Empty;
                btnApply_Click(sender, e);
                baseObj.DbEventWrite(ProcessId, Convert.ToString(Session["SiteCode"]), 21, "TD Apply", 0);
                ...
                this.SchemeDiscCalculation();
                //BindSchemeGrid();
                DDLMaterialCodeNew.SelectedIndex = 0;
                baseObj.DbEventWrite(ProcessId, Convert.ToString(Session["SiteCode"]), 22, "End Scheme", 0);
                BtnAddItem.Enabled = true;
                //DDLProductGroup.Items.Clear();
                ...
                if (isDirectProduct)
                {
                    DDLProductGroupNew.SelectedIndex = 0;
                    DDLProductSubCategoryNew.SelectedIndex = 0;
                }
            }
            catch (Exception ex)
            {
        ...
        {
            ProcessId = DateTime.Now.ToString("yyMMddHHmmssffff");
            baseObj.DbEventWrite(ProcessId, Convert.ToString(Session["SiteCode"]), 101, "Text Change", 0);

            QtyCalcualtion();
            baseObj.DbEventWrite(ProcessId, Convert.ToString(Session["SiteCode"]), 150, "End Text Change", 0);
            BtnAddItem.Enabled = true;
            BtnAddItem.Focus();
            Session["focus"] = 2;
        }
        ...
        private bool Validation()
        {
            ...
            if (ddlShipToAddressNew.SelectedValue == null || ddlShipToAddressNew.SelectedValue.ToString().Trim().Length == 0)
            {
        ...
        private void SaveSaleOrder()
        {
                        ...
                        if (Session["LineItem"] != null)
                        {
                            ...
                            foreach (GridViewRow grv in gvScheme.Rows)
                            {
                                if (((CheckBox)grv.FindControl("chkSelect")).Checked)
                                {
                                    ...
                                    string[] calValue = baseObj.CalculatePrice1(grv.Cells[4].Text, ddlCustomerNew.SelectedItem.Value, Convert.ToDecimal(billQty < 0 ? 1 : billQty), "Box", Session["SiteCode"].ToString());
                                    i = i + 1;
                                    cmd1.Parameters.Clear();
                                    if (billQty > 0)
                                    {
            ...
            {
                ScriptManager.RegisterStartupScript(this,this.GetType(), "blah", "HideLoader();", true);
                if (conn != null)
                {
        ...
        public void TDCalculation()
        {
            try
            {
                ...
                foreach (DataRow row in dtLineItems.Rows)
                {
                    ...
                    if (TD!=Convert.ToDecimal(row["TDValue"]))
                    {
                        TdApplied = true;
                    }
        ...
        private bool ValidatePurchaseReturnHeaderData()
        {
            ...
            try
            {
                ...
                if (ddlCustomerGroupNew.SelectedItem.Value == "Select...")
                {
                ...
                if (ddlPSRNameNew.Visible == true)
                {
                    ...
                    if (ddlBeatNameNew.Text == string.Empty || ddlBeatNameNew.SelectedIndex == -1 || ddlBeatNameNew.SelectedItem.Text == "Select...")
                    {
                        // this.Page.ClientScript.RegisterStartupScript(GetType(), "Alert", " alert('Select Beat Name !');", true);
                ...
                //Customer code
                if (ddlCustomerNew.SelectedIndex == -1)
                {
                ...
                {
                    if (ddlCustomerNew.SelectedItem.Text == "Select...")
                    {
                        //this.Page.ClientScript.RegisterStartupScript(GetType(), "Alert", " alert('Select Customer..');", true);
        ...
        {
            Query3 = "select GSTINNO,GSTREGISTRATIONDATE,COMPOSITIONSCHEME,Address1,STATE,Mobile_No from VW_CUSTOMERGSTINFO where CUSTOMER_CODE='" + ddlCustomerNew.SelectedValue + "'";
            ...
            if (dt1.Rows.Count > 0)
            {
                ...
                //chkCompositionSchemeNew.Checked = Convert.ToBoolean(dt1.Rows[0]["COMPOSITIONSCHEME"]);
            ...
            else
            {
                ...
                //chkCompositionSchemeNew.Checked = false;
                txtBilltoState.Text = string.Empty;
                txtAddress.Text = string.Empty;
                txtContactNo.Text = string.Empty;
            }

            Query2 = "EXEC USP_CUSTSHIPTOADDRESS '" + ddlCustomerNew.SelectedValue + "'";
            ddlShipToAddressNew.Items.Clear();
            baseObj.BindToDropDownp(ddlShipToAddressNew, Query2, "SHIPTOADDRESS", "SHIPTOADDRESS");

            if (ddlBeatNameNew.SelectedIndex > 0)
            {
                Session["LineItem"] = null;
                Session["SchemeLineItem"] = null;
                Session["SoNo"] = null;
            ...
            else
            {
                strQuery = @"Select Ac.Mobile_No,Ac.ADDRESS1,Ac.PSR_CODE,AP.PSR_Name,APB.BeatName,APB.BeatCode,Ac.APPLICABLESCHEMEDISCOUNT,case Ac.APPLICABLESCHEMEDISCOUNT when  0 then 'None' when 1 then 'Scheme' when 2 then 'Discount' when 3 then 'Both' end SchemeDiscount "
                         ...
                         + " where Ac.Customer_Code ='" + ddlCustomerNew.SelectedValue + "' and (Ac.DATAAREAID='" + Session["DATAAREAID"].ToString() + "'  or Ac.DATAAREAID='VMAS')";
                //hint
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    ddlPSRNameNew.Items.Clear();
                    ddlBeatNameNew.Items.Clear();
                    //txtSchemeDisc.Items.Clear();
                    ddlPSRNameNew.Items.Add(new ListItem(dt.Rows[0]["PSR_CODE"].ToString() + "-" + dt.Rows[0]["PSR_Name"].ToString(), dt.Rows[0]["PSR_CODE"].ToString()));
                    ddlBeatNameNew.Items.Add(new ListItem(dt.Rows[0]["BeatCode"].ToString() + "-" + dt.Rows[0]["BeatName"].ToString(), dt.Rows[0]["BeatCode"].ToString()));
                    ddlPSRNameNew.Items[0].Selected = true;
                    ddlBeatNameNew.Items[0].Selected = true;
                    ddlShipToAddressNew.Items[0].Selected = true;
                    ddlBusinessUnitNew.Items[0].Selected = true;
        ...
        {
            strQuery = @"select BeatCode +'-'+BeatName as BeatName,BeatCode from [ax].[ACXPSRBeatMaster] where PSRCode='" + ddlPSRNameNew.SelectedItem.Value + "'";
            ddlBeatNameNew.Items.Clear();
            ddlBeatNameNew.Items.Add("Select...");
            baseObj.BindToDropDownp(ddlBeatNameNew, strQuery, "BeatName", "BeatCode");

        }

        protected void ddlCustomerGroup_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (ddlCustomerGroupNew.SelectedValue == "CG0001")
            {
                ...
                ddlPSRNameNew.Items.Clear();
                ddlPSRNameNew.Items.Add("Select...");
                baseObj.BindToDropDownp(ddlPSRNameNew, strQuery, "PSRName", "PSR_Code");
                //ddlPSRNameNew.Items[0].Selected = true;
                lblBeatName.Visible = true;
                lblPSRName.Visible = true;
                ddlBeatNameNew.Visible = true;
                ddlPSRNameNew.Visible = true;
                ...
                strQuery = "Select distinct Customer_Code + '-' + Customer_Name as Name,Customer_Code from ax.ACXCUSTMASTER CM" +
                                ...
                                " where CM.BLOCKED = 0 AND CM.CUST_GROUP = '" + ddlCustomerGroupNew.SelectedValue + "' AND CM.SITE_CODE = '" + Session["SiteCode"].ToString() + "'" +
            ...
            else
            {
                ddlPSRNameNew.Items.Clear();
                ddlBeatNameNew.Items.Clear();
                ddlBeatNameNew.Visible = false;
                ddlPSRNameNew.Visible = false;
                ...
                //if (ddlCustomerGroup.SelectedValue == "CG0004")
                //{
                strQuery = "Select Customer_Code+'-'+Customer_Name as Name,Customer_Code  from ax.ACXCUSTMASTER where Blocked = 0 AND CUST_GROUP='" + ddlCustomerGroupNew.SelectedValue + "' AND SITE_CODE='" + Session["SiteCode"].ToString() + "'" +
                        " Union  All " +
                        "select B.SUBDISTRIBUTOR +'-'+ A.Customer_Name as Name,B.SUBDISTRIBUTOR Customer_Code  from ax.ACX_SDLinking B LEFT OUTER JOIN AX.ACXCUSTMASTER A ON A.CUSTOMER_CODE=B.SUBDISTRIBUTOR WHERE B.Other_Site='" + Session["SiteCode"].ToString() + "' AND A.CUST_GROUP='" + ddlCustomerGroupNew.SelectedValue + "' AND A.BLOCKED=0 AND B.BLOCKED=0 ";
                //" Select Name=(Select A.Customer_Code+'-'+A.Customer_Name as Name  from ax.ACXCUSTMASTER A where A.Blocked = 0 and B.SubDistributor=A.Customer_Code AND A.CUST_GROUP='" + ddlCustomerGroup.SelectedValue + "' ),B.SubDistributor as Customer_Code " +
                //" from ax.ACX_SDLinking B Where B.Other_Site='" + Session["SiteCode"].ToString() + "'";
                //}
                //else
        ...
        private void ResetAllControls()
        {
            ...
            ddlCustomerNew.Items.Clear();
            ddlCustomerNew.Items.Add("Select...");
            ddlBusinessUnitNew.SelectedIndex = 0;
            lblPSRName.Visible = false;
            ddlPSRNameNew.Items.Clear();
            ddlPSRNameNew.Visible = false;
            lblBeatName.Visible = false;
        ...
        {
            strQuery = @"Select Customer_Code+'-'+Customer_Name as Name,Customer_Code from ax.ACXCUSTMASTER where Blocked = 0 and PSR_CODE ='" + ddlPSRNameNew.SelectedItem.Value + "' and PSR_BEAT='" + ddlBeatNameNew.SelectedItem.Value + "' "
        ...
        protected void DDLProductSubCategory_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                DDLMaterialCodeNew.Enabled = true;
                DDLMaterialCodeNew.Focus();
                PcsBillingApplicable();
                isDirectProduct = false;
            }
        ...
        protected void DDLMaterialCode_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                DataTable dt = new DataTable();
                string query = "select iTEMID,Product_Group,PRODUCT_SUBCATEGORY,cast(Product_PackSize as decimal(10,2)) as Product_PackSize,Cast(Product_Crate_PackSize as decimal(10,2)) as Product_CrateSize,EXEMPT from ax.INVENTTABLE where ItemId='" + DDLMaterialCodeNew.SelectedItem.Value + "'";
                dt = baseObj.GetData(query);

                if (dt.Rows.Count > 0)
                {
                    if (DDLProductGroupNew.Text != dt.Rows[0]["PRODUCT_GROUP"].ToString())
                    {
                        DDLProductGroupNew.Text = dt.Rows[0]["PRODUCT_GROUP"].ToString();
                        ddlProductGroup_SelectedIndexChanged(sender, e);
                        DDLMaterialCodeNew.SelectedValue = dt.Rows[0]["ItemId"].ToString();
                        PcsBillingApplicable();
                    }
                    if (DDLProductSubCategoryNew.Text != dt.Rows[0]["PRODUCT_SUBCATEGORY"].ToString())
                    {
                        DDLProductSubCategoryNew.Text = dt.Rows[0]["PRODUCT_SUBCATEGORY"].ToString();
                    }
        ...
        public void BindSchemeGrid()
        {
            ...
            if (dt != null)
            {
                foreach (DataRow row in dt.Rows)
                {
                    ...
                    dtscheme = baseObj.GetData("EXEC USP_ACX_GetSalesLineCalcGST '" + row["Free Item Code"].ToString() + "','" + ddlCustomerNew.SelectedValue.ToString() + "','" + Session["SiteCode"].ToString() + "',1," + Convert.ToDecimal(row["Rate"].ToString()) + ",'" + Session["SITELOCATION"].ToString() + "','" + ddlCustomerGroupNew.SelectedValue.ToString() + "','" + Session["DATAAREAID"].ToString() + "'");
        ...
        public DataTable GetDatafromSP(string SPName)
        {
            ...
            try
            {
                ...
                cmd.Parameters.AddWithValue("@CUSCODECUS", ddlCustomerNew.SelectedValue.ToString());
                cmd.Parameters.AddWithValue("@CUSCODEGROUP", ddlCustomerGroupNew.SelectedValue.ToString());
        ...
        public Int16 GetQtyofGroupItem(string Group, string BoxPCS)
        {
            ...
            {
                dt = baseObj.GetData("Select  DISTINCT ITEMID From AX.ACXFreeItemGroupTable Where DATAAREAID='" + Session["DATAAREAID"] + "' or DATAAREAID='VMAS'");
            }
            else
            {
                dt = baseObj.GetData("Select DISTINCT ITEMID From AX.ACXFreeItemGroupTable Where [GROUP] ='" + Group + "' and (DATAAREAID='" + Session["DATAAREAID"] + "' or DATAAREAID='VMAS')");
            }
            Int16 Qty = 0;
        ...
        protected void SchemeDiscCalculation()
        {
            try
            {
                ...
                Boolean SchemeDiscApply = false, AddSchemeDiscApply = false,DiscountApplied = false;
                string SchSrlNo = "0", AddSchemeGroup = "", AddSchSrlNo = "0";
                DiscountApplied=SchemeDiscApply = AddSchemeDiscApply = false;
                    ...
                    if (chkBx.Checked && hdnSchemetype.Value.ToString() == "3")
                    {
                        ...
                        if (dtSchValue.Rows.Count > 0)
                        {
                            ...
                            if (dr.Length > 0)
                            {
                                ...
                                {
                                    dt = baseObj.GetData("Select DISTINCT ITEMID From AX.ACXFreeItemGroupTable Where [GROUP] ='" + dr[0]["Scheme Item Group"].ToString().ToUpper() + "' and (DATAAREAID='" + Session["DATAAREAID"] + "'  or DATAAREAID='VMAS')");
                                }
                                else
                                {
                ...
                if (AddSchemeDiscApply == true && (AddSchemePerc > 0 || AddSchemeValue > 0))
                {
                    ...
                    if (dtAddSch.Rows.Count > 0)
                    {
                        ...
                        if (dr.Length > 0)
                        {
                            ...
                            {
                                dt = baseObj.GetData("Select DISTINCT ITEMID From AX.ACXFreeItemGroupTable Where [GROUP] ='" + dr[0]["ADDITIONDISCOUNTITEMGROUP"].ToString().ToUpper() + "' and (DATAAREAID='" + Session["DATAAREAID"] + "'  or DATAAREAID='VMAS')");
                            }
                            else
                            {
                ...
                if (b)
                {
                    if (AsyncFileUpload1.HasFile)
                    {
                        ...
                        for (int i = 0; i < dtExcelData.Rows.Count; i++)
                        {
                            ...
                            ReturnArray = obj.CalculatePrice1(dtExcelData.Rows[i]["ProductCode"].ToString(), ddlCustomerNew.SelectedItem.Value, TotalBoxQty, "Box", Session["SiteCode"].ToString());
                            if (ReturnArray != null && ReturnArray[2].ToString() == "")
                            {
                                dtForShownUnuploadData.Rows.Add();
                                dtForShownUnuploadData.Rows[j]["ProductCode"] = dtExcelData.Rows[i]["ProductCode"].ToString();
                            ...
                            else if (ReturnArray != null && ReturnArray[2].ToString() != "")
                            {
                                ...
                                dt = baseObj.GetData("EXEC USP_ACX_GetSalesLineCalcGST '" + dtExcelData.Rows[i]["ProductCode"].ToString() + "','" + ddlCustomerNew.SelectedValue.ToString() + "','" + Session["SiteCode"].ToString() + "'," + TotalBoxQty + "," + Convert.ToDecimal(ReturnArray[2].ToString()) + ",'" + Session["SITELOCATION"].ToString() + "','" + ddlCustomerGroupNew.SelectedValue.ToString() + "','" + Session["DATAAREAID"].ToString() + "'");
                                if (dt.Rows.Count > 0)
                                {
                                    cmd1.Parameters.AddWithValue("@statusProcedure", "Insert");
                                    cmd1.Parameters.AddWithValue("@SO_NO", PRESO_NO);
        ...
        public void FillProductCode()
        {
            ...
            ilist.Add("@Block"); litem.Add("1");
            ilist.Add("@ProductGroup"); litem.Add(DDLProductGroupNew.SelectedValue.ToString());
            ilist.Add("@ProductSubGroup"); litem.Add(DDLProductSubCategoryNew.SelectedValue.ToString());

            DataTable dtBinddropdown = baseObj.GetData_New(query, CommandType.StoredProcedure, ilist, litem);
            DDLMaterialCodeNew.DataSource = dtBinddropdown;
            DDLMaterialCodeNew.DataTextField = "CODE_DISC";
            DDLMaterialCodeNew.DataValueField = "CODE";
            DDLMaterialCodeNew.DataBind();
            DDLMaterialCodeNew.Items.Insert(0, new ListItem("--Select--", ""));
        }
        //public void FillProductCode()
        //{
        //    if (ddlBusinessUnit.SelectedItem.Text == "All...")
        ...
        protected void QtyCalcualtion()
        {
            ...
            {//===========Validation for zero amount==================
                ...
                //baseObj.DbEventWrite(ProcessId, Convert.ToString(Session["SiteCode"]), 103, "Prod Check", 0);

                if (DDLMaterialCodeNew.SelectedItem.Value == "Select...")
                {
                ...
                baseObj.DbEventWrite(ProcessId, Convert.ToString(Session["SiteCode"]), 104, "Get Rate-" + Convert.ToString(DDLMaterialCodeNew.SelectedItem.Value), 0);
                string query = "EXEC ax.ACX_GetProductRate '" + DDLMaterialCodeNew.SelectedItem.Value + "','" + ddlCustomerNew.SelectedItem.Value + "','" + Session["SiteCode"].ToString() + "'";

                DataTable dtItem = new DataTable();
                dtItem = baseObj.GetData(query);
                baseObj.DbEventWrite(ProcessId, Convert.ToString(Session["SiteCode"]), 105, "Find Rate", dtItem.Rows.Count);
                ...
                if (dtItem.Rows.Count > 0)
                {
                    ...
                    baseObj.DbEventWrite(ProcessId, Convert.ToString(Session["SiteCode"]), 107, "End check", 0);
                    ...
                    baseObj.DbEventWrite(ProcessId, Convert.ToString(Session["SiteCode"]), 107, "Total-" + Convert.ToString(DDLMaterialCodeNew.SelectedItem.Value), Convert.ToInt32(TotalQty));
                    decimal ProductMRP = Convert.ToDecimal(dtItem.Rows[0]["PRODUCT_MRP"].ToString());
                    decimal Litre = Convert.ToDecimal(dtItem.Rows[0]["LTR"].ToString());

                    decimal TotalBox = 0, TotalPcs = 0, price;
                    string BoxPcs = "";
                    ...
                    txtQtyCrates.Text = Math.Round(Math.Ceiling(TotalQty / (crateSize == 0 ? 1 : crateSize)), 2).ToString();
                    txtLtr.Text = Math.Round(((TotalQty * Litre * pacSize) / 1000), 2).ToString();            //Changed Logic on 12 May 2016[Pramod Sir]
                    price = Math.Round(ProductMRP, 2);
                    txtValue.Text = Math.Round((price * TotalQty), 2).ToString();
                    txtPrice.Text = price.ToString();
                    lblHidden.Text= dtItem.Rows[0]["UOM"].ToString();
                    BtnAddItem.Focus();
                    BtnAddItem.CausesValidation = false;

                    //baseObj.DbEventWrite(ProcessId, Convert.ToString(Session["SiteCode"]), 108, "Cal Price-TB:" + TotalBox.ToString("0.00") + "VB" + txtViewTotalBox.Text, 0);
                    ...
                    //baseObj.DbEventWrite(ProcessId, Convert.ToString(Session["SiteCode"]), 109, "End Price" + calValue[0].ToString(), 0);
                }
                else
                {
            ...
            {
                ErrorSignal.FromCurrentContext().Raise(ex);
                baseObj.DbEventWrite(ProcessId, Convert.ToString(Session["SiteCode"]), 149, "Error", 0);
        ...
        private void ShowData_ForExcel()
        {
            ...
            try
            {
                ...
                dt = new DataTable();
                dt.Load(cmd.ExecuteReader());
                string myServerPath = Server.MapPath("~/ExcelTemplate/SaleOrder.xlsx");
                ...
                if (dt != null && dt.Rows.Count > 0)
                {
                    for (int i = 0; i < dt.Rows.Count; i++)
                    {
                        ...
                        //sh.Cells[i + 2, 5] = dt.Rows[i][4];
                        //sh.Cells[i + 2, 6] = dt.Rows[i][5];
                        //sh.Cells[i + 2, 7] = dt.Rows[i][6];
                    }
                }
        ...
        protected void btnrecalculate_Click(object sender, EventArgs e)
        {
            ...
            QtyCalcualtion();
            ScriptManager.RegisterStartupScript(this, typeof(Page), "Validation", "alert('Price re-calculation done.');", true);
            BtnAddItem.Enabled = true;
            BtnAddItem.Focus();
            Session["focus"] = 2;
        }
        ...
        private void BindingGird()
        {
            ...
            baseObj.DbEventWrite(ProcessId, Convert.ToString(Session["SiteCode"]), 11, "Binding Start", 0);
            ...
            if (dt != null)
            {
                if (dt.Rows.Count > 0)
                {
                    ...
                    GridViewFooterCalculate(dt);
                    gvDetails.Visible = true;
                    baseObj.DbEventWrite(ProcessId, Convert.ToString(Session["SiteCode"]), 12, "Binded", dt.Rows.Count);
            ...
            baseObj.DbEventWrite(ProcessId, Convert.ToString(Session["SiteCode"]), 13, "Binding End", 0);
        }
