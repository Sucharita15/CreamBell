...
{
    public partial class frmSaleRegisterPartyWise : System.Web.UI.Page
    {
        ...
        private void ShowReport()
        {
            ...
            try
            {
                ...
                //FilterQuery = " SELECT SITEID,TRANSTYPE, INVOICE_DATE, INVOICE_NO, CUSTOMER_NAME,SIW.PRODUCT_NAME, "
                //            +  "  (SELECT BoxQty FROM dbo.udf_GetPCSDetails(isnull(cast(SIW.Box as numeric), 0), "
                //            +  "  CAST(INVT.PRODUCT_PACKSIZE AS INT))) BOX "
                //            +  " , (SELECT PCSQty FROM dbo.udf_GetPCSDetails(isnull(cast(SIW.Box as numeric), 0), "
                //            +  "  CAST(INVT.PRODUCT_PACKSIZE AS INT))) PCS , "
                //            +  "  (SELECT BoxPrint FROM dbo.udf_GetPCSDetails(isnull(cast(SIW.Box as numeric), 0), "
                //            +  "  CAST(INVT.PRODUCT_PACKSIZE AS INT))) [TotalBoxPCS]  ,  "
                //            +  "  BOX as QtyConv, SIW.LTR, LINEAMOUNT, DISC_AMOUNT, SEC_DISC_AMOUNT, DISC, TAX_CODE, TAX_AMOUNT, ADDTAX_CODE, ADDTAX_AMOUNT , AMOUNT " 
                //            +  " FROM ACX_SALESUMMARY_PARTY_ITEM_WISE SIW"
                ...
                FilterQuery = "EXEC ACX_USP_SaleRegisterPartyWise '" + ucRoleFilters.GetCommaSepartedSiteId() + "','" + Convert.ToDateTime(txtFromDate.Text).ToString("yyyy-MM-dd") + "','" + Convert.ToDateTime(txtToDate.Text).ToString("yyyy-MM-dd") + "','" + CustomerGroup + "','" + Customer + "'";
        ...
        private void LoadDataInReportViewer(DataTable dtSetHeader, DataTable dtSetData)
        {
            try
            {
                ...
                {
                    ReportViewer1.LocalReport.ReportPath = Server.MapPath("Reports\\SaleRegisterPartyWiseDetail.rdl");
